        ...
        axios
            ...
            .then((response) => {
                ...
                const commentHostData = data.COMMENT_HOST ? data.COMMENT_HOST.map(item => {
                    const { cocommentCount, comment, commentId, currentDateTime, modifiedDateTime, nickname, role } = item;
                    return { cocommentCount, comment, commentId, currentDateTime, modifiedDateTime, nickname, role };
                }) : [];
                    ...
                    <PopupContainer >
                        {qnaData.type === Role.HOST && (
                            <>
                                ...
                                <p>{qnaData.content}</p>
                                <button className="btn btn-danger me-3" onClick={handleDeleteQnaPost}>삭제</button>
                                <button className="btn btn-primary">수정</button>
                                <hr />
                                <CommentContainer key={qnaData.commentId} onWheel={handleWheel}>
                                    {qnaData.commentData.sort((a, b) => a.commentId - b.commentId).map(comment => {
                                        if (comment.role === Role.COMMENT_GUEST) {
                                            return (
                                                <>
                                                    <p key={comment.commentId}>{comment.nickname}: {comment.comment} </p>
                                                </>
                                            )
                                        } else if (comment.role === Role.COMMENT_HOST) {
                                            return (
                                                <>
                                                    <p key={comment.commentId}>{comment.nickname}: {comment.comment} <button onClick={() => handleDeleteComment(comment.commentId)}>삭제</button></p>
